...
| `return_taxes` | [`Array<Order Return Tax Hash>`]($m/OrderReturnTax) | Optional | Taxes applied to the `OrderReturnServiceCharge`. By default, return-level taxes apply to<br>`OrderReturnServiceCharge`s calculated in the `SUBTOTAL_PHASE` if `taxable` is set to `true`.  On<br>read or retrieve, this list includes both item-level taxes and any return-level taxes<br>apportioned to this item.<br><br>This field has been deprecated in favour of `applied_taxes`. |
