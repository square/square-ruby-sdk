...
| `idempotency_key` | `String` | Required | A unique string that identifies this `CreatePayment` request. Keys can be any valid string<br>but must be unique for every `CreatePayment` request.<br><br>Note: The number of allowed characters might be less than the stated maximum, if multi-byte<br>characters are used.<br><br>For more information, see [Idempotency](https://developer.squareup.com/docs/working-with-apis/idempotency).<br>**Constraints**: *Minimum Length*: `1`, *Maximum Length*: `45` |
...
| `customer_id` | `String` | Optional | The [Customer](entity:Customer) ID of the customer associated with the payment.<br><br>This is required if the `source_id` refers to a card on file created using the Cards API. |
...
| `team_member_id` | `String` | Optional | An optional [TeamMember](entity:TeamMember) ID to associate with<br>this payment. |
