...
| `type` | [`String (Tender Type)`]($m/TenderType) |  | Indicates a tender's type. |
| `card_details` | [`Tender Card Details Hash`]($m/TenderCardDetails) | Optional | Represents additional details of a tender with `type` `CARD` or `SQUARE_GIFT_CARD` |
| `cash_details` | [`Tender Cash Details Hash`]($m/TenderCashDetails) | Optional | Represents the details of a tender with `type` `CASH`. |
| `additional_recipients` | [`Array<Additional Recipient Hash>`]($m/AdditionalRecipient) | Optional | Additional recipients (other than the merchant) receiving a portion of this tender.<br>For example, fees assessed on the purchase by a third party integration. |
