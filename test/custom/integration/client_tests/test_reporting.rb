...
describe Square::Reporting::Client do
  # A minimal but VALID reporting query. The /reporting/v1/load endpoint rejects
  # an empty query ({}) with HTTP 400 ("Query should contain either measures,
  # dimensions or timeDimensions..."), so we name a measure that exists in the
  # live metadata (mirrors what the other SDKs' live tests query).
  VALID_QUERY = { measures: ["Appointments.count"] }.freeze
  ...
  describe "#load" do
    it "returns data or the Continue-wait sentinel" do
      response = @reporting_client.reporting.load(query: VALID_QUERY)

      refute_nil response
      assert_instance_of Square::Types::LoadResponse, response

      # A still-processing query comes back as the Continue-wait sentinel;
      # otherwise the flat LoadResponse carries `data`. Both are valid here.
      sentinel = response.to_h["error"] == "Continue wait"

      assert(sentinel || !response.data.nil?, "expected data or the Continue-wait sentinel")
  ...
  describe "#load_and_wait" do
    it "polls until the query resolves" do
      response = @reporting_client.reporting.load_and_wait(query: VALID_QUERY)
