...
#  Either `percentage` or `amount_money` should be set, but not both.
  ...
  # @return [SquareApiClient::Money] The amount of money applied to the order by the service charge, including
...
#  tax amount is already included in both `applied_money` and `total_tax_money`.
  ...
  # @return [SquareApiClient::OrderServiceChargeCalculationPhase] The calculation phase after which to apply the service charge.
#  See
...
#  calculated in the `TOTAL_PHASE` cannot be marked as taxable.
    attr_reader :taxable
  # @return [Array<SquareApiClient::OrderLineItemAppliedTax>] The list of references to `OrderReturnTax` entities applied to the
...
#  Either `percentage` or `amount_money` should be set, but not both.
    # @param applied_money [SquareApiClient::Money] The amount of money applied to the order by the service charge, including
#  any inclusive tax amounts, as calculated by Square.
#  - For fixed-amount service charges, `applied_money` is equal to `amount_money`.
#  - For percentage-based service charges, `applied_money` is the money calculated
#  using the percentage.
...
#  tax amount is already included in both `applied_money` and `total_tax_money`.
    ...
    # @param calculation_phase [SquareApiClient::OrderServiceChargeCalculationPhase] The calculation phase after which to apply the service charge.
#  See
#  [OrderServiceChargeCalculationPhase](#type-orderservicechargecalculationphase)
#  for possible values
    # @param taxable [Boolean] Indicates whether the surcharge can be taxed. Service charges
#  calculated in the `TOTAL_PHASE` cannot be marked as taxable.
    # @param applied_taxes [Array<SquareApiClient::OrderLineItemAppliedTax>] The list of references to `OrderReturnTax` entities applied to the
#  `OrderReturnServiceCharge`. Each `OrderLineItemAppliedTax` has a `tax_uid`
#  that references the `uid` of a top-level `OrderReturnTax` that is being
#  applied to the `OrderReturnServiceCharge`. On reads, the applied amount is
#  populated.
...
#  See [OrderServiceChargeScope](#type-orderservicechargescope) for possible values
    ...
    # @return [SquareApiClient::OrderReturnServiceCharge]
