...
#  Either `errors` or `locations` is present in a given response (never both).
  ...
  # @return [Array<SquareApiClient::Error>] Any errors that occurred during the request.
    attr_reader :errors
  # @return [Array<SquareApiClient::Location>] The business locations.
  ...
  # @return [Object] 
    ...
    # @param locations [Array<SquareApiClient::Location>] The business locations.
...
# Deserialize a JSON object to an instance of ListLocationsResponse
  ...
  item = item.to_json
  SquareApiClient::Location.from_json(json_object: item)
end
      new(
        errors: errors,
        locations: locations,
