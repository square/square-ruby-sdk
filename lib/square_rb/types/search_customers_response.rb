...
#  Either `errors` or `customers` is present in a given response (never both).
  ...
  # @return [Array<SquareApiClient::Error>] Any errors that occurred during the request.
    attr_reader :errors
  # @return [Array<SquareApiClient::Customer>] The customer profiles that match the search query. If any search condition is
...
#  present only if `count` is set to `true` in the request.
  ...
  # @return [Object] 
    ...
    # @param customers [Array<SquareApiClient::Customer>] The customer profiles that match the search query. If any search condition is
#  not met, the result is an empty object (`{}`).
#  Only customer profiles with public information (`given_name`, `family_name`,
#  `company_name`, `email_address`, or `phone_number`)
#  are included in the response.
