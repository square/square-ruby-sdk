...
#  Default: false
  ...
  # @return [SquareApiClient::PaymentOptionsDelayAction] The action to be applied to the `Payment` when the delay_duration has elapsed.
...
#  docs/payments-api/take-payments/card-payments/partial-payments-with-gift-cards).
#  Default: false
    # @param delay_action [SquareApiClient::PaymentOptionsDelayAction] The action to be applied to the `Payment` when the delay_duration has elapsed.
#  The action must be CANCEL or COMPLETE.
#  The action cannot be set to COMPLETE if an `order_id` is present on the
#  TerminalCheckout.
#  This parameter can only be set for a delayed capture payment
...
#  See [DelayAction](#type-delayaction) for possible values
    ...
    # @return [SquareApiClient::PaymentOptions]
...
#  definitions.
    ...
    def self.validate_raw(obj:)
      ...
      obj.delay_action&.is_a?(SquareApiClient::PaymentOptionsDelayAction) != false || raise("Passed value for field obj.delay_action is not the expected type, validation failed.")
    end
  end
end