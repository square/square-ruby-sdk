...
#  com/docs/customers-api/use-the-api/search-customers#search-by-custom-attribute).
  ...
  # @return [Array<SquareApiClient::CustomerCustomAttributeFilter>] The custom attribute filters. Each filter must specify `key` and include the
...
#  list of custom attribute filters.
  ...
  # @return [Object] 
    ...
    # @param filters [Array<SquareApiClient::CustomerCustomAttributeFilter>] The custom attribute filters. Each filter must specify `key` and include the
#  `filter` field with a type-specific filter,
#  the `updated_at` field, or both. The provided keys must be unique within the
#  list of custom attribute filters.
    # @param additional_properties [OpenStruct] Additional properties unmapped to the current class definition
    # @return [SquareApiClient::CustomerCustomAttributeFilters]
...
# Deserialize a JSON object to an instance of CustomerCustomAttributeFilters
  ...
  item = item.to_json
  SquareApiClient::CustomerCustomAttributeFilter.from_json(json_object: item)
end
