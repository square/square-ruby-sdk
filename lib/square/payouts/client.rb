...
#  the seller.
    # @param status [SquareApiClient::PayoutStatus] If provided, only payouts with the given status are returned.
...
#  Default: The current time.
    # @param sort_order [SquareApiClient::SortOrder] The order in which payouts are listed.
...
#  api.payouts.list
  ...
  req.url "#{@request_client.get_url(request_options: request_options)}/v2/payouts"
      ...
      SquareApiClient::ListPayoutsResponse.from_json(json_object: response.body)
    end
...
#  To call this endpoint, set `PAYOUTS_READ` for the OAuth scope.
    ...
    # @param request_options [SquareApiClient::RequestOptions] 
    # @return [SquareApiClient::GetPayoutResponse]
    # @example
#  api = SquareApiClient::Client.new(
#    base_url: "https://api.example.com",
#    environment: SquareApiClient::Environment::PRODUCTION,
...
#  api.payouts.get(payout_id: "payout_id")
  ...
  req.url "#{@request_client.get_url(request_options: request_options)}/v2/payouts/#{payout_id}"
      ...
      SquareApiClient::GetPayoutResponse.from_json(json_object: response.body)
    end
# Retrieves a list of all payout entries for a specific payout.
#  To call this endpoint, set `PAYOUTS_READ` for the OAuth scope.
    #
    # @param payout_id [String] The ID of the payout to retrieve the information for.
    # @param sort_order [SquareApiClient::SortOrder] The order in which payout entries are listed.
    # @param cursor [String] A pagination cursor returned by a previous call to this endpoint.
#  Provide this cursor to retrieve the next set of results for the original query.
#  For more information, see
#  ttps://developer.squareup.com/docs/build-basics/common-api-patterns/pagination).
...
#  api.payouts.list_entries(payout_id: "payout_id")
  ...
  req.url "#{@request_client.get_url(request_options: request_options)}/v2/payouts/#{payout_id}/payout-entries"
      ...
      SquareApiClient::ListPayoutEntriesResponse.from_json(json_object: response.body)
    end
  end
