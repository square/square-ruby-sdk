...
#  [SearchCustomers](api-endpoint:Customers-SearchCustomers) request.
  class CustomerQuery
  # @return [square.rb::CustomerFilter] The filtering criteria for the search query. A query can contain multiple
...
#  searches and join the result sets.
    attr_reader :filter
  # @return [square.rb::CustomerSort] Sorting criteria for query results. The default behavior is to sort
#  customers alphabetically by `given_name` and `family_name`.
  ...
  # @return [Object] 
    ...
    # @param filter [square.rb::CustomerFilter] The filtering criteria for the search query. A query can contain multiple
#  filters in any combination.
#  Multiple filters are combined as `AND` statements.
#  __Note:__ Combining multiple filters as `OR` statements is not supported.
#  Instead, send multiple single-filter
#  searches and join the result sets.
    # @param sort [square.rb::CustomerSort] Sorting criteria for query results. The default behavior is to sort
#  customers alphabetically by `given_name` and `family_name`.
    # @param additional_properties [OpenStruct] Additional properties unmapped to the current class definition
    # @return [square.rb::CustomerQuery]
...
# Deserialize a JSON object to an instance of CustomerQuery
    ...
    def self.from_json(json_object:)
      ...
      unless parsed_json["filter"].nil?
        ...
        filter = square.rb::CustomerFilter.from_json(json_object: filter)
      else
        filter = nil
      end
      unless parsed_json["sort"].nil?
        sort = parsed_json["sort"].to_json
        sort = square.rb::CustomerSort.from_json(json_object: sort)
      else
        sort = nil
      end
      new(
...
#  definitions.
    ...
    def self.validate_raw(obj:)
      obj.filter.nil? || square.rb::CustomerFilter.validate_raw(obj: obj.filter)
      obj.sort.nil? || square.rb::CustomerSort.validate_raw(obj: obj.sort)
    end
  end
end