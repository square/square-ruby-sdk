...
# Represents a response from deleting one or more order custom attributes.
  ...
  # @return [Array<square.rb::Error>] Any errors that occurred during the request.
    attr_reader :errors
  # @return [Hash{String => square.rb::DeleteOrderCustomAttributeResponse}] A map of responses that correspond to individual delete requests. Each response
#  has the same ID
...
#  `errors` field.
  ...
  # @return [Object] 
    ...
    # @param values [Hash{String => square.rb::DeleteOrderCustomAttributeResponse}] A map of responses that correspond to individual delete requests. Each response
#  has the same ID
#  as the corresponding request and contains either a `custom_attribute` or an
#  `errors` field.
...
#  BulkDeleteOrderCustomAttributesResponse
  ...
  value = value.to_json
  square.rb::DeleteOrderCustomAttributeResponse.from_json(json_object: value)
end
      new(
        errors: errors,
        values: values,
