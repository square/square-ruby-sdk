...
#  response.
  ...
  # @return [Array<square.rb::Error>] Any errors that occurred during the request.
    attr_reader :errors
  # @return [square.rb::LoyaltyEvent] The resulting loyalty event. Starting in Square version 2022-08-17, this field
#  is no longer returned.
    attr_reader :event
  # @return [Array<square.rb::LoyaltyEvent>] The resulting loyalty events. If the purchase qualifies for points, the
...
#  if the purchase also qualifies for a loyalty promotion.
  ...
  # @return [Object] 
    ...
    # @param event [square.rb::LoyaltyEvent] The resulting loyalty event. Starting in Square version 2022-08-17, this field
#  is no longer returned.
    # @param events [Array<square.rb::LoyaltyEvent>] The resulting loyalty events. If the purchase qualifies for points, the
#  `ACCUMULATE_POINTS` event
#  is always included. When using the Orders API, the `ACCUMULATE_PROMOTION_POINTS`
#  event is included
#  if the purchase also qualifies for a loyalty promotion.
...
# Deserialize a JSON object to an instance of AccumulateLoyaltyPointsResponse
  ...
  item = item.to_json
  square.rb::LoyaltyEvent.from_json(json_object: item)
end
      new(
        errors: errors,
        event: event,
