...
#  is otherwise an opaque id generated from key device fields.
    attr_reader :id
  # @return [SquareApiClient::DeviceAttributes] A collection of DeviceAttributes representing the device.
    attr_reader :attributes
  # @return [Array<SquareApiClient::Component>] A list of components applicable to the device.
    attr_reader :components
  # @return [SquareApiClient::DeviceStatus] The current status of the device.
    attr_reader :status
...
#  prefix and
#  is otherwise an opaque id generated from key device fields.
    # @param attributes [SquareApiClient::DeviceAttributes] A collection of DeviceAttributes representing the device.
    # @param components [Array<SquareApiClient::Component>] A list of components applicable to the device.
    # @param status [SquareApiClient::DeviceStatus] The current status of the device.
    # @param additional_properties [OpenStruct] Additional properties unmapped to the current class definition
    # @return [SquareApiClient::Device]
...
# Deserialize a JSON object to an instance of Device
    ...
    def self.from_json(json_object:)
      ...
      unless parsed_json["attributes"].nil?
        ...
        attributes = SquareApiClient::DeviceAttributes.from_json(json_object: attributes)
      else
        attributes = nil
      end
      components = parsed_json["components"]&.map do | item |
  item = item.to_json
  SquareApiClient::Component.from_json(json_object: item)
end
      unless parsed_json["status"].nil?
        status = parsed_json["status"].to_json
        status = SquareApiClient::DeviceStatus.from_json(json_object: status)
      else
        status = nil
      end
      new(
...
#  definitions.
    ...
    def self.validate_raw(obj:)
      ...
      SquareApiClient::DeviceAttributes.validate_raw(obj: obj.attributes)
      obj.components&.is_a?(Array) != false || raise("Passed value for field obj.components is not the expected type, validation failed.")
      obj.status.nil? || SquareApiClient::DeviceStatus.validate_raw(obj: obj.status)
    end
  end
end