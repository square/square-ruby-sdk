...
# Represents the details of a tender with `type` `BUY_NOW_PAY_LATER`.
  ...
  # @return [SquareApiClient::TenderBuyNowPayLaterDetailsBrand] The Buy Now Pay Later brand.
#  See [Brand](#type-brand) for possible values
    attr_reader :buy_now_pay_later_brand
  # @return [SquareApiClient::TenderBuyNowPayLaterDetailsStatus] The buy now pay later payment's current state (such as `AUTHORIZED` or
#  `CAPTURED`). See
...
#  See [Status](#type-status) for possible values
  ...
  # @return [Object] 
    ...
    # @param buy_now_pay_later_brand [SquareApiClient::TenderBuyNowPayLaterDetailsBrand] The Buy Now Pay Later brand.
#  See [Brand](#type-brand) for possible values
    # @param status [SquareApiClient::TenderBuyNowPayLaterDetailsStatus] The buy now pay later payment's current state (such as `AUTHORIZED` or
#  `CAPTURED`). See
#  [TenderBuyNowPayLaterDetailsStatus](entity:TenderBuyNowPayLaterDetailsStatus)
#  for possible values.
#  See [Status](#type-status) for possible values
    # @param additional_properties [OpenStruct] Additional properties unmapped to the current class definition
    # @return [SquareApiClient::TenderBuyNowPayLaterDetails]
...
#  definitions.
    ...
    def self.validate_raw(obj:)
      obj.buy_now_pay_later_brand&.is_a?(SquareApiClient::TenderBuyNowPayLaterDetailsBrand) != false || raise("Passed value for field obj.buy_now_pay_later_brand is not the expected type, validation failed.")
      obj.status&.is_a?(SquareApiClient::TenderBuyNowPayLaterDetailsStatus) != false || raise("Passed value for field obj.status is not the expected type, validation failed.")
    end
  end
end