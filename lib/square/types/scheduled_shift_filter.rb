...
#  to get location IDs.
  ...
  # @return [square.rb::TimeRange] Return shifts whose `start_at` time is within the specified
#  time range (inclusive).
    attr_reader :start
  # @return [square.rb::TimeRange] Return shifts whose `end_at` time is within the specified
#  time range (inclusive).
    attr_reader :end_
  # @return [square.rb::ScheduledShiftWorkday] Return shifts based on a workday date range.
    attr_reader :workday
...
#  returned along with shifts assigned to the specified team members.
  ...
  # @return [square.rb::ScheduledShiftFilterAssignmentStatus] Return shifts based on whether a team member is assigned. A shift is
...
#  for possible values
  ...
  # @return [Array<square.rb::ScheduledShiftFilterScheduledShiftStatus>] Return shifts based on the draft or published status of the shift.
...
#  [ListLocations](api-endpoint:Locations-ListLocations)
#  to get location IDs.
    # @param start [square.rb::TimeRange] Return shifts whose `start_at` time is within the specified
#  time range (inclusive).
    # @param end_ [square.rb::TimeRange] Return shifts whose `end_at` time is within the specified
#  time range (inclusive).
    # @param workday [square.rb::ScheduledShiftWorkday] Return shifts based on a workday date range.
    # @param team_member_ids [Array<String>] Return shifts assigned to specified team members. If needed, call
#  [SearchTeamMembers](api-endpoint:Team-SearchTeamMembers) to get team member IDs.
#  To return only the shifts assigned to the specified team members, include the
#  `assignment_status` filter in the query. Otherwise, all unassigned shifts are
#  returned along with shifts assigned to the specified team members.
    # @param assignment_status [square.rb::ScheduledShiftFilterAssignmentStatus] Return shifts based on whether a team member is assigned. A shift is
#  assigned if the `team_member_id` field is populated in the `draft_shift_details`
#  or `published_shift details` field of the shift.
#  To return only draft or published shifts, include the `scheduled_shift_statuses`
#  filter in the query.
...
#  cheduledShiftFilterAssignmentStatus](#type-scheduledshiftfilterassignmentstatus)
#  for possible values
    # @param scheduled_shift_statuses [Array<square.rb::ScheduledShiftFilterScheduledShiftStatus>] Return shifts based on the draft or published status of the shift.
#  A shift is published if the `published_shift_details` field is present.
#  Note that shifts with `draft_shift_details.is_deleted` set to `true` are ignored
#  with the `DRAFT` filter.
#  See
...
#  for possible values
    ...
    # @return [square.rb::ScheduledShiftFilter]
...
# Deserialize a JSON object to an instance of ScheduledShiftFilter
    ...
    def self.from_json(json_object:)
      ...
      unless parsed_json["workday"].nil?
        ...
        workday = square.rb::ScheduledShiftWorkday.from_json(json_object: workday)
      else
        workday = nil
      end
...
#  definitions.
    ...
    def self.validate_raw(obj:)
      ...
      obj.workday.nil? || square.rb::ScheduledShiftWorkday.validate_raw(obj: obj.workday)
      ...
      obj.assignment_status&.is_a?(square.rb::ScheduledShiftFilterAssignmentStatus) != false || raise("Passed value for field obj.assignment_status is not the expected type, validation failed.")
