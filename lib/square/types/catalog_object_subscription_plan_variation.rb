...
require_relative "catalog_subscription_plan_variation"
require_relative "catalog_v_1_id"
require "ostruct"
require "json"

module square.rb
  class CatalogObjectSubscriptionPlanVariation
  # @return [String] 
    attr_reader :type
  # @return [square.rb::CatalogSubscriptionPlanVariation] Structured data for a `CatalogSubscriptionPlanVariation`, set for CatalogObjects
#  of type `SUBSCRIPTION_PLAN_VARIATION`.
    attr_reader :subscription_plan_variation_data
...
#  time.
  ...
  # @return [Hash{String => square.rb::CatalogCustomAttributeValue}] A map (key-value pairs) of application-defined custom attribute values. The
...
#  details, etc.).
  ...
  # @return [Array<square.rb::CatalogV1Id>] The Connect v1 IDs for this object at each location where it is present, where
#  they
...
#  This can include locations that are deactivated.
  ...
  # @return [Object] 
    ...
    # @param type [String] 
    # @param subscription_plan_variation_data [square.rb::CatalogSubscriptionPlanVariation] Structured data for a `CatalogSubscriptionPlanVariation`, set for CatalogObjects
#  of type `SUBSCRIPTION_PLAN_VARIATION`.
...
#  being inserted. When deleted, the `updated_at` field will equal the deletion
#  time.
    # @param custom_attribute_values [Hash{String => square.rb::CatalogCustomAttributeValue}] A map (key-value pairs) of application-defined custom attribute values. The
#  value of a key-value pair
#  is a [CatalogCustomAttributeValue](entity:CatalogCustomAttributeValue) object.
#  The key is the `key` attribute
#  value defined in the associated
...
#  to store any sensitive information (personally identifiable information, card
#  details, etc.).
    # @param catalog_v_1_ids [Array<square.rb::CatalogV1Id>] The Connect v1 IDs for this object at each location where it is present, where
#  they
#  differ from the object's Connect V2 ID. The field will only be present for
#  objects that
#  have been created or modified by legacy APIs.
...
#  This can include locations that are deactivated.
    ...
    # @return [square.rb::CatalogObjectSubscriptionPlanVariation]
    def initialize(type:, subscription_plan_variation_data: OMIT, id:, updated_at: OMIT, version: OMIT, is_deleted: OMIT, custom_attribute_values: OMIT, catalog_v_1_ids: OMIT, present_at_all_locations: OMIT, present_at_location_ids: OMIT, absent_at_location_ids: OMIT, image_id: OMIT, additional_properties: nil)
      @type = type
      @subscription_plan_variation_data = subscription_plan_variation_data if subscription_plan_variation_data != OMIT
      @id = id
...
#  CatalogObjectSubscriptionPlanVariation
    ...
    def self.from_json(json_object:)
      ...
      unless parsed_json["subscription_plan_variation_data"].nil?
        subscription_plan_variation_data = parsed_json["subscription_plan_variation_data"].to_json
        subscription_plan_variation_data = square.rb::CatalogSubscriptionPlanVariation.from_json(json_object: subscription_plan_variation_data)
      else
        subscription_plan_variation_data = nil
      end
      id = parsed_json["id"]
  ...
  value = value.to_json
  square.rb::CatalogCustomAttributeValue.from_json(json_object: value)
end
      catalog_v_1_ids = parsed_json["catalog_v1_ids"]&.map do | item |
  item = item.to_json
  square.rb::CatalogV1Id.from_json(json_object: item)
end
      ...
      new(
        type: type,
        subscription_plan_variation_data: subscription_plan_variation_data,
        id: id,
...
# Serialize an instance of CatalogObjectSubscriptionPlanVariation to a JSON object
    #
    # @return [String]
    def to_json
      @_field_set&.to_json
...
#  definitions.
    ...
    def self.validate_raw(obj:)
      ...
      obj.subscription_plan_variation_data.nil? || square.rb::CatalogSubscriptionPlanVariation.validate_raw(obj: obj.subscription_plan_variation_data)
