...
#  Either `scheduled_shifts` or `errors` is present in the response.
  ...
  # @return [Array<SquareApiClient::ScheduledShift>] A paginated list of scheduled shifts that match the query conditions.
...
#  only if additional results are available.
  ...
  # @return [Object] 
    ...
    # @param scheduled_shifts [Array<SquareApiClient::ScheduledShift>] A paginated list of scheduled shifts that match the query conditions.
...
# Deserialize a JSON object to an instance of SearchScheduledShiftsResponse
  ...
  item = item.to_json
  SquareApiClient::ScheduledShift.from_json(json_object: item)
end
