...
#  the [Customer Custom Attributes API](api:CustomerCustomAttributes).
    attr_reader :key
  # @return [square.rb::CustomerCustomAttributeFilterValue] A filter that corresponds to the data type of the target custom attribute. For
...
#  You must provide this `filter` field, the `updated_at` field, or both.
    attr_reader :filter
  # @return [square.rb::TimeRange] The date range for when the custom attribute was last updated. The date range
...
#  (and the corresponding custom attribute definition) and can be retrieved using
#  the [Customer Custom Attributes API](api:CustomerCustomAttributes).
    # @param filter [square.rb::CustomerCustomAttributeFilterValue] A filter that corresponds to the data type of the target custom attribute. For
#  example, provide the `phone` filter to
#  search based on the value of a `PhoneNumber`-type custom attribute. The data
#  type is specified by the schema field of the custom attribute definition,
#  which can be retrieved using the [Customer Custom Attributes
#  API](api:CustomerCustomAttributes).
#  You must provide this `filter` field, the `updated_at` field, or both.
    # @param updated_at [square.rb::TimeRange] The date range for when the custom attribute was last updated. The date range
#  can include `start_at`, `end_at`, or
#  both. Range boundaries are inclusive. Dates are specified as RFC 3339
#  timestamps.
#  You must provide this `updated_at` field, the `filter` field, or both.
    # @param additional_properties [OpenStruct] Additional properties unmapped to the current class definition
    # @return [square.rb::CustomerCustomAttributeFilter]
...
# Deserialize a JSON object to an instance of CustomerCustomAttributeFilter
    ...
    def self.from_json(json_object:)
      ...
      unless parsed_json["filter"].nil?
        ...
        filter = square.rb::CustomerCustomAttributeFilterValue.from_json(json_object: filter)
      else
        filter = nil
      end
...
#  definitions.
    ...
    def self.validate_raw(obj:)
      ...
      obj.filter.nil? || square.rb::CustomerCustomAttributeFilterValue.validate_raw(obj: obj.filter)
