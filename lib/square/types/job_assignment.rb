...
#  setting](entity:WageSetting).
  ...
  # @return [SquareApiClient::JobAssignmentPayType] The current pay type for the job assignment used to
...
#  See [JobAssignmentPayType](#type-jobassignmentpaytype) for possible values
  ...
  # @return [SquareApiClient::Money] The hourly pay rate of the job. For `SALARY` pay types, Square calculates the
#  hourly rate based on
#  `annual_rate` and `weekly_hours`.
    attr_reader :hourly_rate
  # @return [SquareApiClient::Money] The total pay amount for a 12-month period on the job. Set if the job `PayType`
#  is `SALARY`.
  ...
  # @return [Object] 
    ...
    # @param pay_type [SquareApiClient::JobAssignmentPayType] The current pay type for the job assignment used to
#  calculate the pay amount in a pay period.
#  See [JobAssignmentPayType](#type-jobassignmentpaytype) for possible values
    # @param hourly_rate [SquareApiClient::Money] The hourly pay rate of the job. For `SALARY` pay types, Square calculates the
#  hourly rate based on
#  `annual_rate` and `weekly_hours`.
    # @param annual_rate [SquareApiClient::Money] The total pay amount for a 12-month period on the job. Set if the job `PayType`
#  is `SALARY`.
    ...
    # @return [SquareApiClient::JobAssignment]
...
# Deserialize a JSON object to an instance of JobAssignment
    ...
    def self.from_json(json_object:)
      ...
      unless parsed_json["annual_rate"].nil?
        ...
        annual_rate = SquareApiClient::Money.from_json(json_object: annual_rate)
      else
        annual_rate = nil
      end
...
#  definitions.
    ...
    def self.validate_raw(obj:)
      ...
      obj.pay_type.is_a?(SquareApiClient::JobAssignmentPayType) != false || raise("Passed value for field obj.pay_type is not the expected type, validation failed.")
      obj.hourly_rate.nil? || SquareApiClient::Money.validate_raw(obj: obj.hourly_rate)
      obj.annual_rate.nil? || SquareApiClient::Money.validate_raw(obj: obj.annual_rate)
