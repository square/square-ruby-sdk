...
#  of a customer-related [custom attribute](entity:CustomAttribute).
  ...
  # @return [SquareApiClient::CustomerTextFilter] A filter for a query based on the value of an `Email`-type custom attribute.
...
#  matches steven.jones@gmail.com and mygmail@stevensbakery.com.
    attr_reader :email
  # @return [SquareApiClient::CustomerTextFilter] A filter for a query based on the value of a `PhoneNumber`-type custom
...
#  +14151234567, +12345674158, and +1234156780.
    attr_reader :phone
  # @return [SquareApiClient::CustomerTextFilter] A filter for a query based on the value of a `String`-type custom attribute.
...
#  dog". However, `quick foxes` and `qui` do not match.
    attr_reader :text
  # @return [SquareApiClient::FilterValue] A filter for a query based on the display name for a `Selection`-type custom
...
#  names. These UUIDs are unique per seller.
  ...
  # @return [SquareApiClient::TimeRange] A filter for a query based on the value of a `Date`-type custom attribute.
...
#  in `YYYY-MM-DD` format or as RFC 3339 timestamps.
    attr_reader :date
  # @return [SquareApiClient::FloatNumberRange] A filter for a query based on the value of a `Number`-type custom attribute,
...
#  `(2^63-1)/10^5`, or 92233720368547.
  ...
  # @return [Boolean] A filter for a query based on the value of a `Boolean`-type custom attribute.
    attr_reader :boolean
  # @return [SquareApiClient::CustomerAddressFilter] A filter for a query based on the value of an `Address`-type custom attribute.
#  The filter can include `postal_code`, `country`, or both.
  ...
  # @return [Object] 
    ...
    # @param email [SquareApiClient::CustomerTextFilter] A filter for a query based on the value of an `Email`-type custom attribute.
#  This filter is case-insensitive and can
#  include `exact` or `fuzzy`, but not both.
#  For an `exact` match, provide the complete email address.
#  For a `fuzzy` match, provide a query expression containing one or more query
...
#  irrespective of the token order. For example, `Steven gmail`
#  matches steven.jones@gmail.com and mygmail@stevensbakery.com.
    # @param phone [SquareApiClient::CustomerTextFilter] A filter for a query based on the value of a `PhoneNumber`-type custom
#  attribute. This filter is case-insensitive and
#  can include `exact` or `fuzzy`, but not both.
#  For an `exact` match, provide the complete phone number. This is always an
#  E.164-compliant phone number that starts
...
#  Similarly, the expression `415` matches
#  +14151234567, +12345674158, and +1234156780.
    # @param text [SquareApiClient::CustomerTextFilter] A filter for a query based on the value of a `String`-type custom attribute.
#  This filter is case-insensitive and
#  can include `exact` or `fuzzy`, but not both.
#  For an `exact` match, provide the complete string.
#  For a `fuzzy` match, provide a query expression containing one or more query
...
#  `brown quick`, and `quick fox` match "The quick brown fox jumps over the lazy
#  dog". However, `quick foxes` and `qui` do not match.
    # @param selection [SquareApiClient::FilterValue] A filter for a query based on the display name for a `Selection`-type custom
#  attribute value. This filter is case-sensitive
#  and can contain `any`, `all`, or both. The `none` condition is not supported.
#  Provide the display name of each item that you want to search for. To find the
#  display names for the selection, use the
...
#  or more UUIDs (sourced from the `schema.items.enum` field) that map to the item
#  names. These UUIDs are unique per seller.
    # @param date [SquareApiClient::TimeRange] A filter for a query based on the value of a `Date`-type custom attribute.
#  Provide a date range for this filter using `start_at`, `end_at`, or both. Range
#  boundaries are inclusive. Dates can be specified
#  in `YYYY-MM-DD` format or as RFC 3339 timestamps.
    # @param number [SquareApiClient::FloatNumberRange] A filter for a query based on the value of a `Number`-type custom attribute,
#  which can be an integer or a decimal with up to
#  5 digits of precision.
#  Provide a numerical range for this filter using `start_at`, `end_at`, or both.
#  Range boundaries are inclusive. Numbers are specified
...
#  `(2^63-1)/10^5`, or 92233720368547.
    ...
    # @param address [SquareApiClient::CustomerAddressFilter] A filter for a query based on the value of an `Address`-type custom attribute.
#  The filter can include `postal_code`, `country`, or both.
    # @param additional_properties [OpenStruct] Additional properties unmapped to the current class definition
    # @return [SquareApiClient::CustomerCustomAttributeFilterValue]
...
# Deserialize a JSON object to an instance of CustomerCustomAttributeFilterValue
    ...
    def self.from_json(json_object:)
      ...
      unless parsed_json["email"].nil?
        ...
        email = SquareApiClient::CustomerTextFilter.from_json(json_object: email)
      else
        email = nil
      end
      unless parsed_json["phone"].nil?
        phone = parsed_json["phone"].to_json
        phone = SquareApiClient::CustomerTextFilter.from_json(json_object: phone)
      else
        phone = nil
      end
      unless parsed_json["text"].nil?
        text = parsed_json["text"].to_json
        text = SquareApiClient::CustomerTextFilter.from_json(json_object: text)
      else
        text = nil
      end
      unless parsed_json["selection"].nil?
        selection = parsed_json["selection"].to_json
        selection = SquareApiClient::FilterValue.from_json(json_object: selection)
      else
        selection = nil
      end
      unless parsed_json["date"].nil?
        date = parsed_json["date"].to_json
        date = SquareApiClient::TimeRange.from_json(json_object: date)
      else
        date = nil
      end
      unless parsed_json["number"].nil?
        number = parsed_json["number"].to_json
        number = SquareApiClient::FloatNumberRange.from_json(json_object: number)
      else
        number = nil
      end
      ...
      unless parsed_json["address"].nil?
        ...
        address = SquareApiClient::CustomerAddressFilter.from_json(json_object: address)
      else
        address = nil
      end
      new(
...
#  definitions.
    ...
    def self.validate_raw(obj:)
      obj.email.nil? || SquareApiClient::CustomerTextFilter.validate_raw(obj: obj.email)
      obj.phone.nil? || SquareApiClient::CustomerTextFilter.validate_raw(obj: obj.phone)
      obj.text.nil? || SquareApiClient::CustomerTextFilter.validate_raw(obj: obj.text)
      obj.selection.nil? || SquareApiClient::FilterValue.validate_raw(obj: obj.selection)
      obj.date.nil? || SquareApiClient::TimeRange.validate_raw(obj: obj.date)
      obj.number.nil? || SquareApiClient::FloatNumberRange.validate_raw(obj: obj.number)
      obj.boolean&.is_a?(Boolean) != false || raise("Passed value for field obj.boolean is not the expected type, validation failed.")
      obj.address.nil? || SquareApiClient::CustomerAddressFilter.validate_raw(obj: obj.address)
    end
  end
end