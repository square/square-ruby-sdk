...
#  and time, but it cannot be set for the future.
  ...
  # @return [square.rb::LoyaltyAccountMapping] The mapping that associates the loyalty account with a buyer. Currently,
...
#  in the `phone_number` field.
    attr_reader :mapping
  # @return [Array<square.rb::LoyaltyAccountExpiringPointDeadline>] The schedule for when points expire in the loyalty account balance. This field
...
#  and time, but it cannot be set for the future.
    ...
    # @param mapping [square.rb::LoyaltyAccountMapping] The mapping that associates the loyalty account with a buyer. Currently,
#  a loyalty account can only be mapped to a buyer by phone number.
#  To create a loyalty account, you must specify the `mapping` field, with the
#  buyer's phone number
#  in the `phone_number` field.
    # @param expiring_point_deadlines [Array<square.rb::LoyaltyAccountExpiringPointDeadline>] The schedule for when points expire in the loyalty account balance. This field
#  is present only if the account has points that are scheduled to expire.
#  The total number of points in this field equals the number of points in the
#  `balance` field.
    # @param additional_properties [OpenStruct] Additional properties unmapped to the current class definition
    # @return [square.rb::LoyaltyAccount]
...
# Deserialize a JSON object to an instance of LoyaltyAccount
    ...
    def self.from_json(json_object:)
      ...
      unless parsed_json["mapping"].nil?
        ...
        mapping = square.rb::LoyaltyAccountMapping.from_json(json_object: mapping)
      else
        mapping = nil
      end
      expiring_point_deadlines = parsed_json["expiring_point_deadlines"]&.map do | item |
  item = item.to_json
  square.rb::LoyaltyAccountExpiringPointDeadline.from_json(json_object: item)
end
      new(
        id: id,
        program_id: program_id,
...
#  definitions.
    ...
    def self.validate_raw(obj:)
      ...
      obj.mapping.nil? || square.rb::LoyaltyAccountMapping.validate_raw(obj: obj.mapping)
