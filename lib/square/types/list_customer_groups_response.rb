...
#  Either `errors` or `groups` is present in a given response (never both).
  ...
  # @return [Array<square.rb::Error>] Any errors that occurred during the request.
    attr_reader :errors
  # @return [Array<square.rb::CustomerGroup>] A list of customer groups belonging to the current seller.
    attr_reader :groups
...
#  ttps://developer.squareup.com/docs/build-basics/common-api-patterns/pagination).
  ...
  # @return [Object] 
    ...
    # @param groups [Array<square.rb::CustomerGroup>] A list of customer groups belonging to the current seller.
...
# Deserialize a JSON object to an instance of ListCustomerGroupsResponse
  ...
  item = item.to_json
  square.rb::CustomerGroup.from_json(json_object: item)
end
