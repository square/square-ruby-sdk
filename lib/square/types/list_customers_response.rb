...
#  Either `errors` or `customers` is present in a given response (never both).
  ...
  # @return [Array<square.rb::Error>] Any errors that occurred during the request.
    attr_reader :errors
  # @return [Array<square.rb::Customer>] The customer profiles associated with the Square account or an empty object
#  (`{}`) if none are found.
...
#  only if `count` is set to `true` in the request.
  ...
  # @return [Object] 
    ...
    # @param customers [Array<square.rb::Customer>] The customer profiles associated with the Square account or an empty object
#  (`{}`) if none are found.
#  Only customer profiles with public information (`given_name`, `family_name`,
#  `company_name`, `email_address`, or
#  `phone_number`) are included in the response.
