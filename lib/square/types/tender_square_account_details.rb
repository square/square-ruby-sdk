...
# Represents the details of a tender with `type` `SQUARE_ACCOUNT`.
  ...
  # @return [square.rb::TenderSquareAccountDetailsStatus] The Square Account payment's current state (such as `AUTHORIZED` or
#  `CAPTURED`). See
...
#  See [Status](#type-status) for possible values
  ...
  # @return [Object] 
    ...
    # @param status [square.rb::TenderSquareAccountDetailsStatus] The Square Account payment's current state (such as `AUTHORIZED` or
#  `CAPTURED`). See
#  [TenderSquareAccountDetailsStatus](entity:TenderSquareAccountDetailsStatus)
#  for possible values.
#  See [Status](#type-status) for possible values
    # @param additional_properties [OpenStruct] Additional properties unmapped to the current class definition
    # @return [square.rb::TenderSquareAccountDetails]
...
#  definitions.
    ...
    def self.validate_raw(obj:)
      obj.status&.is_a?(square.rb::TenderSquareAccountDetailsStatus) != false || raise("Passed value for field obj.status is not the expected type, validation failed.")
    end
  end
end