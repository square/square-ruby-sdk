...
# A query filter to search for buyer-accessible availabilities by.
  ...
  # @return [SquareApiClient::TimeRange] The query expression to search for buy-accessible availabilities with their
...
#  This query expression cannot be set if `booking_id` is set.
  ...
  # @return [Array<SquareApiClient::SegmentFilter>] The query expression to search for buyer-accessible availabilities matching the
...
#  cannot be set.
  ...
  # @return [Object] 
    ...
    # @param start_at_range [SquareApiClient::TimeRange] The query expression to search for buy-accessible availabilities with their
#  starting times falling within the specified time range.
#  The time range must be at least 24 hours and at most 32 days long.
#  For waitlist availabilities, the time range can be 0 or more up to 367 days
#  long.
...
#  location IDs matching the specified location ID.
#  This query expression cannot be set if `booking_id` is set.
    # @param segment_filters [Array<SquareApiClient::SegmentFilter>] The query expression to search for buyer-accessible availabilities matching the
#  specified list of segment filters.
#  If the size of the `segment_filters` list is `n`, the search returns
#  availabilities with `n` segments per availability.
#  This query expression cannot be set if `booking_id` is set.
...
#  cannot be set.
    ...
    # @return [SquareApiClient::SearchAvailabilityFilter]
...
# Deserialize a JSON object to an instance of SearchAvailabilityFilter
    ...
    def self.from_json(json_object:)
      ...
      unless parsed_json["start_at_range"].nil?
        ...
        start_at_range = SquareApiClient::TimeRange.from_json(json_object: start_at_range)
      else
        start_at_range = nil
      end
  ...
  item = item.to_json
  SquareApiClient::SegmentFilter.from_json(json_object: item)
end
...
#  definitions.
    ...
    def self.validate_raw(obj:)
      SquareApiClient::TimeRange.validate_raw(obj: obj.start_at_range)
