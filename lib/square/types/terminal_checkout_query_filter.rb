...
#  `TerminalCheckout` objects for the merchant are displayed.
  ...
  # @return [square.rb::TimeRange] The time range for the beginning of the reporting period, which is inclusive.
...
#  is specified, then all
#  `TerminalCheckout` objects for the merchant are displayed.
    # @param created_at [square.rb::TimeRange] The time range for the beginning of the reporting period, which is inclusive.
#  Default value: The current time minus one day.
#  Note that `TerminalCheckout`s are available for 30 days after creation.
    # @param status [String] Filtered results with the desired status of the `TerminalCheckout`.
#  Options: `PENDING`, `IN_PROGRESS`, `CANCEL_REQUESTED`, `CANCELED`, `COMPLETED`
    # @param additional_properties [OpenStruct] Additional properties unmapped to the current class definition
    # @return [square.rb::TerminalCheckoutQueryFilter]
