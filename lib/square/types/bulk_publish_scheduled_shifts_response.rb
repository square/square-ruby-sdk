...
#  Either `scheduled_shifts` or `errors` is present in the response.
  ...
  # @return [Hash{String => square.rb::PublishScheduledShiftResponse}] A map of key-value pairs that represent responses for individual publish
#  requests.
...
#  any errors that occurred while processing the request.
  ...
  # @return [Array<square.rb::Error>] Any top-level errors that prevented the bulk operation from succeeding.
    attr_reader :errors
  ...
  # @return [Object] 
    ...
    # @param responses [Hash{String => square.rb::PublishScheduledShiftResponse}] A map of key-value pairs that represent responses for individual publish
#  requests.
#  The order of responses might differ from the order in which the requests were
#  provided.
#  - Each key is the scheduled shift ID that was specified for a publish request.
...
#  containing
#  any errors that occurred while processing the request.
    # @param errors [Array<square.rb::Error>] Any top-level errors that prevented the bulk operation from succeeding.
...
# Deserialize a JSON object to an instance of BulkPublishScheduledShiftsResponse
  ...
  value = value.to_json
  square.rb::PublishScheduledShiftResponse.from_json(json_object: value)
end
