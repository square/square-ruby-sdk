...
#  discount.
  ...
  # @return [String] The discount's name.
    attr_reader :name
  # @return [square.rb::OrderLineItemDiscountType] The type of the discount. If it is created by the API, it is `FIXED_PERCENTAGE`
#  or `FIXED_AMOUNT`.
...
#  `percentage` is not set for amount-based discounts.
  ...
  # @return [square.rb::Money] The total declared monetary amount of the discount.
#  `amount_money` is not set for percentage-based discounts.
    attr_reader :amount_money
  # @return [square.rb::Money] The amount of discount actually applied to this line item. When an amount-based
...
#  is distributed across the line items.
  ...
  # @return [square.rb::OrderLineItemDiscountScope] Indicates the level at which the `OrderReturnDiscount` applies. For `ORDER`
#  scoped
...
#  discount.
    ...
    # @param type [square.rb::OrderLineItemDiscountType] The type of the discount. If it is created by the API, it is `FIXED_PERCENTAGE`
#  or `FIXED_AMOUNT`.
#  Discounts that do not reference a catalog object ID must have a type of
#  `FIXED_PERCENTAGE` or `FIXED_AMOUNT`.
#  See [OrderLineItemDiscountType](#type-orderlineitemdiscounttype) for possible
...
#  A value of `"7.25"` corresponds to a percentage of 7.25%.
#  `percentage` is not set for amount-based discounts.
    # @param amount_money [square.rb::Money] The total declared monetary amount of the discount.
#  `amount_money` is not set for percentage-based discounts.
    # @param applied_money [square.rb::Money] The amount of discount actually applied to this line item. When an amount-based
#  discount is at the order level, this value is different from `amount_money`
#  because the discount
#  is distributed across the line items.
    # @param scope [square.rb::OrderLineItemDiscountScope] Indicates the level at which the `OrderReturnDiscount` applies. For `ORDER`
#  scoped
#  discounts, the server generates references in `applied_discounts` on all
#  `OrderReturnLineItem`s. For `LINE_ITEM` scoped discounts, the discount is only
#  applied to
...
#  values
    ...
    # @return [square.rb::OrderReturnDiscount]
...
#  definitions.
    ...
    def self.validate_raw(obj:)
      ...
      obj.type&.is_a?(square.rb::OrderLineItemDiscountType) != false || raise("Passed value for field obj.type is not the expected type, validation failed.")
      ...
      obj.scope&.is_a?(square.rb::OrderLineItemDiscountScope) != false || raise("Passed value for field obj.scope is not the expected type, validation failed.")
    end
  end
end