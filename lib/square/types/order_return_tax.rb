...
#  order subtotal.
  ...
  # @return [String] The tax's name.
    attr_reader :name
  # @return [square.rb::OrderLineItemTaxType] Indicates the calculation method used to apply the tax.
...
#  For example, a value of `"7.25"` corresponds to a percentage of 7.25%.
  ...
  # @return [square.rb::Money] The amount of money applied by the tax in an order.
    attr_reader :applied_money
  # @return [square.rb::OrderLineItemTaxScope] Indicates the level at which the `OrderReturnTax` applies. For `ORDER` scoped
...
#  See [OrderLineItemTaxScope](#type-orderlineitemtaxscope) for possible values
  ...
  # @return [Object] 
    ...
    # @param type [square.rb::OrderLineItemTaxType] Indicates the calculation method used to apply the tax.
#  See [OrderLineItemTaxType](#type-orderlineitemtaxtype) for possible values
    # @param percentage [String] The percentage of the tax, as a string representation of a decimal number.
#  For example, a value of `"7.25"` corresponds to a percentage of 7.25%.
    # @param applied_money [square.rb::Money] The amount of money applied by the tax in an order.
    # @param scope [square.rb::OrderLineItemTaxScope] Indicates the level at which the `OrderReturnTax` applies. For `ORDER` scoped
#  taxes, Square generates references in `applied_taxes` on all
#  `OrderReturnLineItem`s. For `LINE_ITEM` scoped taxes, the tax is only applied to
#  `OrderReturnLineItem`s with references in their `applied_discounts` field.
#  See [OrderLineItemTaxScope](#type-orderlineitemtaxscope) for possible values
    # @param additional_properties [OpenStruct] Additional properties unmapped to the current class definition
    # @return [square.rb::OrderReturnTax]
...
#  definitions.
    ...
    def self.validate_raw(obj:)
      ...
      obj.type&.is_a?(square.rb::OrderLineItemTaxType) != false || raise("Passed value for field obj.type is not the expected type, validation failed.")
      ...
      obj.scope&.is_a?(square.rb::OrderLineItemTaxScope) != false || raise("Passed value for field obj.scope is not the expected type, validation failed.")
    end
  end
end