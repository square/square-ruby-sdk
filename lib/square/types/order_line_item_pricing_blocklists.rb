...
#  unts](https://developer.squareup.com/docs/orders-api/apply-taxes-and-discounts).
  ...
  # @return [Array<square.rb::OrderLineItemPricingBlocklistsBlockedDiscount>] A list of discounts blocked from applying to the line item.
#  Discounts can be blocked by the `discount_uid` (for ad hoc discounts) or
#  the `discount_catalog_object_id` (for catalog discounts).
    attr_reader :blocked_discounts
  # @return [Array<square.rb::OrderLineItemPricingBlocklistsBlockedTax>] A list of taxes blocked from applying to the line item.
...
#  the `tax_catalog_object_id` (for catalog taxes).
  ...
  # @return [Object] 
    ...
    # @param blocked_discounts [Array<square.rb::OrderLineItemPricingBlocklistsBlockedDiscount>] A list of discounts blocked from applying to the line item.
#  Discounts can be blocked by the `discount_uid` (for ad hoc discounts) or
#  the `discount_catalog_object_id` (for catalog discounts).
    # @param blocked_taxes [Array<square.rb::OrderLineItemPricingBlocklistsBlockedTax>] A list of taxes blocked from applying to the line item.
#  Taxes can be blocked by the `tax_uid` (for ad hoc taxes) or
#  the `tax_catalog_object_id` (for catalog taxes).
    # @param additional_properties [OpenStruct] Additional properties unmapped to the current class definition
    # @return [square.rb::OrderLineItemPricingBlocklists]
...
# Deserialize a JSON object to an instance of OrderLineItemPricingBlocklists
  ...
  item = item.to_json
  square.rb::OrderLineItemPricingBlocklistsBlockedDiscount.from_json(json_object: item)
end
      blocked_taxes = parsed_json["blocked_taxes"]&.map do | item |
  item = item.to_json
  square.rb::OrderLineItemPricingBlocklistsBlockedTax.from_json(json_object: item)
end
      new(
