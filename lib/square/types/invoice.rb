...
#  [RetrieveOrder](api-endpoint:Orders-RetrieveOrder) endpoint using the order ID.
  ...
  # @return [square.rb::InvoiceRecipient] The customer who receives the invoice. This customer data is displayed on the
...
#  `customer_id`.
  ...
  # @return [Array<square.rb::InvoicePaymentRequest>] The payment schedule for the invoice, represented by one or more payment
#  requests that
...
#  //developer.squareup.com/docs/invoices-api/overview#invoices-plus-subscription).
  ...
  # @return [square.rb::InvoiceDeliveryMethod] The delivery method that Square uses to send the invoice, reminders, and
#  receipts to
...
#  (if one is defined).
  ...
  # @return [square.rb::Money] The current amount due for the invoice. In addition to the
#  amount due on the next payment request, this includes any overdue payment
#  amounts.
    attr_reader :next_payment_amount_money
  # @return [square.rb::InvoiceStatus] The status of the invoice.
...
#  of 2021-03-10T08:00:00Z).
  ...
  # @return [square.rb::InvoiceAcceptedPaymentMethods] The payment methods that customers can use to pay the invoice on the
#  Square-hosted
...
#  payment method to `true`.
  ...
  # @return [Array<square.rb::InvoiceCustomField>] Additional seller-defined fields that are displayed on the invoice. For more
#  information, see
#  [Custom
...
#  automatic payments. The default value is `false`.
  ...
  # @return [Array<square.rb::InvoiceAttachment>] Metadata about the attachments on the invoice. Invoice attachments are managed
#  using the
...
#  To view the line items and other information for the associated order, call the
#  [RetrieveOrder](api-endpoint:Orders-RetrieveOrder) endpoint using the order ID.
    # @param primary_recipient [square.rb::InvoiceRecipient] The customer who receives the invoice. This customer data is displayed on the
#  invoice and used by Square to deliver the invoice.
#  This field is required to publish an invoice, and it must specify the
#  `customer_id`.
    # @param payment_requests [Array<square.rb::InvoicePaymentRequest>] The payment schedule for the invoice, represented by one or more payment
#  requests that
#  define payment settings, such as amount due and due date. An invoice supports
#  the following payment request combinations:
#  - One balance
...
#  [Invoices Plus
#  //developer.squareup.com/docs/invoices-api/overview#invoices-plus-subscription).
    # @param delivery_method [square.rb::InvoiceDeliveryMethod] The delivery method that Square uses to send the invoice, reminders, and
#  receipts to
#  the customer. After the invoice is published, Square processes the invoice based
#  on the delivery
#  method and payment request settings, either immediately or on the `scheduled_at`
...
#  date
#  (if one is defined).
    # @param next_payment_amount_money [square.rb::Money] The current amount due for the invoice. In addition to the
#  amount due on the next payment request, this includes any overdue payment
#  amounts.
    # @param status [square.rb::InvoiceStatus] The status of the invoice.
#  See [InvoiceStatus](#type-invoicestatus) for possible values
...
#  of 2021-03-10T08:00:00Z).
    ...
    # @param accepted_payment_methods [square.rb::InvoiceAcceptedPaymentMethods] The payment methods that customers can use to pay the invoice on the
#  Square-hosted
#  invoice page. This setting is independent of any automatic payment requests for
#  the invoice.
#  This field is required when creating an invoice and must set at least one
#  payment method to `true`.
    # @param custom_fields [Array<square.rb::InvoiceCustomField>] Additional seller-defined fields that are displayed on the invoice. For more
#  information, see
#  [Custom
#  ields](https://developer.squareup.com/docs/invoices-api/overview#custom-fields).
#  Adding custom fields to an invoice requires an
...
#  invoice payment page. Stored payment information can be used for future
#  automatic payments. The default value is `false`.
    # @param attachments [Array<square.rb::InvoiceAttachment>] Metadata about the attachments on the invoice. Invoice attachments are managed
#  using the
#  [CreateInvoiceAttachment](api-endpoint:Invoices-CreateInvoiceAttachment) and
#  [DeleteInvoiceAttachment](api-endpoint:Invoices-DeleteInvoiceAttachment)
#  endpoints.
...
# Deserialize a JSON object to an instance of Invoice
    ...
    def self.from_json(json_object:)
      ...
      unless parsed_json["primary_recipient"].nil?
        ...
        primary_recipient = square.rb::InvoiceRecipient.from_json(json_object: primary_recipient)
      else
        primary_recipient = nil
      end
      payment_requests = parsed_json["payment_requests"]&.map do | item |
  item = item.to_json
  square.rb::InvoicePaymentRequest.from_json(json_object: item)
end
      ...
      unless parsed_json["next_payment_amount_money"].nil?
        ...
        next_payment_amount_money = square.rb::Money.from_json(json_object: next_payment_amount_money)
      else
      ...
      unless parsed_json["accepted_payment_methods"].nil?
        ...
        accepted_payment_methods = square.rb::InvoiceAcceptedPaymentMethods.from_json(json_object: accepted_payment_methods)
      else
        accepted_payment_methods = nil
      end
      custom_fields = parsed_json["custom_fields"]&.map do | item |
  item = item.to_json
  square.rb::InvoiceCustomField.from_json(json_object: item)
end
  ...
  item = item.to_json
  square.rb::InvoiceAttachment.from_json(json_object: item)
end
...
#  definitions.
    ...
    def self.validate_raw(obj:)
      ...
      obj.primary_recipient.nil? || square.rb::InvoiceRecipient.validate_raw(obj: obj.primary_recipient)
      ...
      obj.delivery_method&.is_a?(square.rb::InvoiceDeliveryMethod) != false || raise("Passed value for field obj.delivery_method is not the expected type, validation failed.")
      ...
      obj.next_payment_amount_money.nil? || square.rb::Money.validate_raw(obj: obj.next_payment_amount_money)
      obj.status&.is_a?(square.rb::InvoiceStatus) != false || raise("Passed value for field obj.status is not the expected type, validation failed.")
      ...
      obj.accepted_payment_methods.nil? || square.rb::InvoiceAcceptedPaymentMethods.validate_raw(obj: obj.accepted_payment_methods)
