...
#  type](entity:GiftCardActivityType).
  ...
  # @return [square.rb::Money] The amount deducted from the gift card for the redemption. This value is a
#  positive integer.
...
#  related to an order or payment.
  ...
  # @return [square.rb::GiftCardActivityRedeemStatus] The status of the gift card redemption. Gift cards redeemed from Square Point of
#  Sale or the
...
#  See [Status](#type-status) for possible values
  ...
  # @return [Object] 
    ...
    # @param amount_money [square.rb::Money] The amount deducted from the gift card for the redemption. This value is a
#  positive integer.
#  Applications that use a custom payment processing system must specify this
#  amount in the
#  [CreateGiftCardActivity](api-endpoint:GiftCardActivities-CreateGiftCardActivity)
...
#  track information
#  related to an order or payment.
    # @param status [square.rb::GiftCardActivityRedeemStatus] The status of the gift card redemption. Gift cards redeemed from Square Point of
#  Sale or the
#  Square Seller Dashboard use a two-state process: `PENDING`
#  to `COMPLETED` or `PENDING` to  `CANCELED`. Gift cards redeemed using the Gift
#  Card Activities API
...
#  See [Status](#type-status) for possible values
    ...
    # @return [square.rb::GiftCardActivityRedeem]
...
#  definitions.
    ...
    def self.validate_raw(obj:)
      ...
      obj.status&.is_a?(square.rb::GiftCardActivityRedeemStatus) != false || raise("Passed value for field obj.status is not the expected type, validation failed.")
    end
  end
end