...
#  filters, and its value length is of Unicode code points.
    attr_reader :name
  # @return [square.rb::TaxCalculationPhase] Whether the tax is calculated based on a payment's subtotal or total.
#  See [TaxCalculationPhase](#type-taxcalculationphase) for possible values
    attr_reader :calculation_phase
  # @return [square.rb::TaxInclusionType] Whether the tax is `ADDITIVE` or `INCLUSIVE`.
...
#  filters, and its value length is of Unicode code points.
    # @param calculation_phase [square.rb::TaxCalculationPhase] Whether the tax is calculated based on a payment's subtotal or total.
#  See [TaxCalculationPhase](#type-taxcalculationphase) for possible values
    # @param inclusion_type [square.rb::TaxInclusionType] Whether the tax is `ADDITIVE` or `INCLUSIVE`.
#  See [TaxInclusionType](#type-taxinclusiontype) for possible values
...
#  definitions.
    ...
    def self.validate_raw(obj:)
      ...
      obj.calculation_phase&.is_a?(square.rb::TaxCalculationPhase) != false || raise("Passed value for field obj.calculation_phase is not the expected type, validation failed.")
      obj.inclusion_type&.is_a?(square.rb::TaxInclusionType) != false || raise("Passed value for field obj.inclusion_type is not the expected type, validation failed.")
