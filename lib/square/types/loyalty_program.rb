...
#  the loyalty program do not modify the identifier.
    attr_reader :id
  # @return [square.rb::LoyaltyProgramStatus] Whether the program is currently active.
#  See [LoyaltyProgramStatus](#type-loyaltyprogramstatus) for possible values
    attr_reader :status
  # @return [Array<square.rb::LoyaltyProgramRewardTier>] The list of rewards for buyers, sorted by ascending points.
    attr_reader :reward_tiers
  # @return [square.rb::LoyaltyProgramExpirationPolicy] If present, details for how points expire.
    attr_reader :expiration_policy
  # @return [square.rb::LoyaltyProgramTerminology] A cosmetic name for the “points” currency.
  ...
  # @return [Array<square.rb::LoyaltyProgramAccrualRule>] Defines how buyers can earn loyalty points from the base loyalty program.
...
#  the loyalty program do not modify the identifier.
    # @param status [square.rb::LoyaltyProgramStatus] Whether the program is currently active.
#  See [LoyaltyProgramStatus](#type-loyaltyprogramstatus) for possible values
    # @param reward_tiers [Array<square.rb::LoyaltyProgramRewardTier>] The list of rewards for buyers, sorted by ascending points.
    # @param expiration_policy [square.rb::LoyaltyProgramExpirationPolicy] If present, details for how points expire.
    # @param terminology [square.rb::LoyaltyProgramTerminology] A cosmetic name for the “points” currency.
    ...
    # @param accrual_rules [Array<square.rb::LoyaltyProgramAccrualRule>] Defines how buyers can earn loyalty points from the base loyalty program.
#  To check for associated [loyalty promotions](entity:LoyaltyPromotion) that
#  enable
#  buyers to earn extra points, call
#  [ListLoyaltyPromotions](api-endpoint:Loyalty-ListLoyaltyPromotions).
    # @param additional_properties [OpenStruct] Additional properties unmapped to the current class definition
    # @return [square.rb::LoyaltyProgram]
...
# Deserialize a JSON object to an instance of LoyaltyProgram
  ...
  item = item.to_json
  square.rb::LoyaltyProgramRewardTier.from_json(json_object: item)
end
      unless parsed_json["expiration_policy"].nil?
        expiration_policy = parsed_json["expiration_policy"].to_json
        expiration_policy = square.rb::LoyaltyProgramExpirationPolicy.from_json(json_object: expiration_policy)
      else
        expiration_policy = nil
      end
      unless parsed_json["terminology"].nil?
        terminology = parsed_json["terminology"].to_json
        terminology = square.rb::LoyaltyProgramTerminology.from_json(json_object: terminology)
      else
        terminology = nil
      end
  ...
  item = item.to_json
  square.rb::LoyaltyProgramAccrualRule.from_json(json_object: item)
end
      new(
        id: id,
        status: status,
...
#  definitions.
    ...
    def self.validate_raw(obj:)
      ...
      obj.status&.is_a?(square.rb::LoyaltyProgramStatus) != false || raise("Passed value for field obj.status is not the expected type, validation failed.")
      ...
      obj.expiration_policy.nil? || square.rb::LoyaltyProgramExpirationPolicy.validate_raw(obj: obj.expiration_policy)
      obj.terminology.nil? || square.rb::LoyaltyProgramTerminology.validate_raw(obj: obj.terminology)
